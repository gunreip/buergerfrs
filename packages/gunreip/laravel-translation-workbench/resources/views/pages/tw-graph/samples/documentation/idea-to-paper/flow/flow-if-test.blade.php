<section class="mt-4 grid min-w-0 gap-4 lg:grid-cols-2">
    <flux:callout color="indigo" icon="file-text" class="min-w-0">
        <flux:callout.heading>{{ __('Flow if test') }}</flux:callout.heading>
        <flux:callout.text>{{ __('Handmade flow example with individually adjustable components.') }}</flux:callout.text>
        @php
            $docExampleSource = file_get_contents(\Illuminate\Support\Facades\View::getFinder()->find(
                'translation-workbench::pages.tw-graph.samples.documentation.idea-to-paper.flow.flow-if-test',
            ));
            if (preg_match('/^[ \t]*\{\{-- flow-if-test-example-1:start --\}\}\R(.*?)^[ \t]*\{\{-- flow-if-test-example-1:end --\}\}/ms', $docExampleSource, $docExample1Match) !== 1) {
                throw new \LogicException('Missing flow-if-test-example-1 source markers.');
            }
        ...
        <flux:heading class="mt-4" size="sm">{{ __('Flow if test props') }}</flux:heading>
                ...
                >
                    ...
                    <x-translation-workbench::ui.tw-graph.strang.if-else-endif
                        ...
                        :end-label="[
                            ...
                            'placement' => 'node',
                            'side' => 'right',
                            'attachTo' => 'literature.flow.1.if-test.elseif.4.arc-south-east.node.end',
                            'width' => 'half',
                            'align' => 'left',
                        ]"
                    />
                    ...
                    <x-translation-workbench::ui.tw-graph.strang.if-else-endif
                        ...
                        side="right"
                        attach-to="literature.flow.1.if-test-origin.anchorNode-end"
                        start-bridge-length="7.75rem"
                        condition-bridge-length="1.75rem"
                        end-continuation="none"
                        :intro-label="['text' => ['IF'], 'placement' => 'node', 'width' => 'half', 'align' => 'center']"
                        :if-condition-label="[
                            'text' => ['IF color=red', 'THEN RGB(240, 62, 62)'],
                            'width' => 'default',
                            'align' => 'left',
                        ...
                        :end-label="[
                            'text' => ['ENDIF'],
                            'placement' => 'node',
                            'attachTo' => 'literature.flow.1.if-test-right.elseif.4.arc-south-west.node.end',
                            'width' => 'half',
                            'align' => 'center',
                        ]"
                    />
                </x-translation-workbench::ui.tw-graph>
                {{-- flow-if-test-example-1:end --}}
                    </div>
                </x-translation-workbench::ui.tw-graph.preview-tools>
        <flux:field class="m-3 flex justify-end font-mono text-xs text-zinc-400">.../flow/flow-if-test.blade.php</flux:field>
    </flux:callout>
</section>
