<section class="mt-4 grid min-w-0 gap-4 lg:grid-cols-2">
    <flux:callout color="indigo" icon="file-text" class="min-w-0">
        ...
        <flux:callout.text>{{ __('Flow start defines only the entry into an Ablaufdiagramm. The public API mirrors trunk-start: one start label, one first anchor node, and optional left/right labels. Internally it still delegates to parts.start, so the existing part and segment chain remains the single geometry source.') }}</flux:callout.text>
        @php
            $docExampleSource = file_get_contents(\Illuminate\Support\Facades\View::getFinder()->find(
                'translation-workbench::pages.tw-graph.samples.documentation.idea-to-paper.flow.flow-start',
            ));
            if (preg_match('/^[ \t]*\{\{-- flow-start-example-1:start --\}\}\R(.*?)^[ \t]*\{\{-- flow-start-example-1:end --\}\}/ms', $docExampleSource, $docExample1Match) !== 1) {
                throw new \LogicException('Missing flow-start-example-1 source markers.');
            }
        ...
        <flux:heading class="mt-4" size="sm">{{ __('Flow start props') }}</flux:heading>
    ...
    <flux:callout color="zinc" icon="eye" class="min-w-0">
        ...
        <x-translation-workbench::ui.tw-graph.preview-tools :dev="$dev ?? true" :coordinates="$coordinates ?? false">
                    <div class="mt-3 overflow-x-auto rounded-lg border border-zinc-200 bg-white/70 dark:border-zinc-700 dark:bg-zinc-900/40">
                        {{-- flow-start-example-1:start --}}
                <x-translation-workbench::ui.tw-graph
                    class="px-20 py-12"
                    graph-id="idea-to-paper-step-08-flow"
                    :dev="true"
                    :coordinates="true"
                    color="cyan"
                ...
                {{-- flow-start-example-1:end --}}
                    </div>
                </x-translation-workbench::ui.tw-graph.preview-tools>
        <flux:field class="m-3 flex justify-end font-mono text-xs text-zinc-400">.../flow/flow-start.blade.php</flux:field>
    </flux:callout>
</section>
