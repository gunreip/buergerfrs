<section class="mt-4 grid min-w-0 gap-4 lg:grid-cols-2">
    <flux:callout color="indigo" icon="file-text" class="min-w-0">
        <flux:callout.heading>{{ __('Flow if endif') }}</flux:callout.heading>
        <flux:callout.text>{{ __('IF / ENDIF uses the same wrapper with a single IF condition and no additional condition rows. The output arc leads directly into the closing ENDIF label. A separate right-facing section attaches to that ENDIF output anchor.') }}</flux:callout.text>
        @php
            $docExampleSource = file_get_contents(\Illuminate\Support\Facades\View::getFinder()->find(
                'translation-workbench::pages.tw-graph.samples.documentation.idea-to-paper.flow.flow-if-endif',
            ));
            if (preg_match('/^[ \t]*\{\{-- flow-if-endif-example-1:start --\}\}\R(.*?)^[ \t]*\{\{-- flow-if-endif-example-1:end --\}\}/ms', $docExampleSource, $docExample1Match) !== 1) {
                throw new \LogicException('Missing flow-if-endif-example-1 source markers.');
            }
        ...
        <flux:heading class="mt-4" size="sm">{{ __('Flow if endif props') }}</flux:heading>
    ...
    <flux:callout color="zinc" icon="eye" class="min-w-0">
        ...
        <x-translation-workbench::ui.tw-graph.preview-tools :dev="$dev ?? true" :coordinates="$coordinates ?? false">
                ...
                {{-- flow-if-endif-example-1:end --}}
                    </div>
                </x-translation-workbench::ui.tw-graph.preview-tools>
        <flux:field class="m-3 flex justify-end font-mono text-xs text-zinc-400">.../flow/flow-if-endif.blade.php</flux:field>
    </flux:callout>
</section>
