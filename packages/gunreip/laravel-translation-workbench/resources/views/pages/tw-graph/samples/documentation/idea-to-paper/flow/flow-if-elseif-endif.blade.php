<section class="mt-4 grid min-w-0 gap-4 lg:grid-cols-2">
    <flux:callout color="indigo" icon="file-text" class="min-w-0">
        <flux:callout.heading>{{ __('Flow if elseif endif') }}</flux:callout.heading>
        <flux:callout.text>{{ __('Two individually authored IF / ELSEIF / ENDIF components share the same starting anchor. Each block has its own labels, conditions, lengths, and direction settings.') }}</flux:callout.text>
        @php
            $docExampleSource = file_get_contents(\Illuminate\Support\Facades\View::getFinder()->find(
                'translation-workbench::pages.tw-graph.samples.documentation.idea-to-paper.flow.flow-if-elseif-endif',
            ));
            if (preg_match('/^[ \t]*\{\{-- flow-if-elseif-endif-example-1:start --\}\}\R(.*?)^[ \t]*\{\{-- flow-if-elseif-endif-example-1:end --\}\}/ms', $docExampleSource, $docExample1Match) !== 1) {
                throw new \LogicException('Missing flow-if-elseif-endif-example-1 source markers.');
            }
        ...
        <flux:heading class="mt-4" size="sm">{{ __('Flow if elseif endif props') }}</flux:heading>
    ...
    <flux:callout color="zinc" icon="eye" class="min-w-0">
        ...
        <x-translation-workbench::ui.tw-graph.preview-tools :dev="$dev ?? true" :coordinates="$coordinates ?? false">
                    <div class="mt-3 overflow-x-auto rounded-lg border border-zinc-200 bg-white/70 dark:border-zinc-700 dark:bg-zinc-900/40">
                        {{-- flow-if-elseif-endif-example-1:start --}}
                <x-translation-workbench::ui.tw-graph
                    class="px-20 py-12"
                    graph-id="idea-to-paper-step-08-flow-if-elseif-endif"
                    :dev="true"
                    :coordinates="true"
                    color="cyan"
                ...
                >
                    <x-translation-workbench::ui.tw-graph.strang.flow-start
                        id="literature.flow.1.if-elseif-endif-origin"
                    ...
                    <x-translation-workbench::ui.tw-graph.strang.if-else-endif
                        id="literature.flow.1.if-elseif-endif"
                        side="left"
                        attach-to="literature.flow.1.if-elseif-endif-origin.anchorNode-end"
                    ...
                    {{-- IF ELSE ENDIF Right --}}
                    <x-translation-workbench::ui.tw-graph.strang.if-else-endif
                        id="literature.flow.1.if-elseif-endif-right"
                        side="right"
                        attach-to="literature.flow.1.if-elseif-endif-origin.anchorNode-end"
                        start-bridge-length="12.75rem"
                        condition-bridge-length="1.75rem"
                ...
                {{-- flow-if-elseif-endif-example-1:end --}}
                    </div>
                </x-translation-workbench::ui.tw-graph.preview-tools>
        <flux:field class="m-3 flex justify-end font-mono text-xs text-zinc-400">.../flow/flow-if-elseif-endif.blade.php</flux:field>
    </flux:callout>
</section>
