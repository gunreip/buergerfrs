<section class="mt-4 grid min-w-0 gap-4 lg:grid-cols-2">
    <flux:callout color="indigo" icon="file-text" class="min-w-0">
        <flux:callout.heading>{{ __('Flow if else endif') }}</flux:callout.heading>
        <flux:callout.text>{{ __('The reusable IF / ELSE / ENDIF wrapper starts from an existing flow anchor. It keeps the handmade graph readable by wiring the intro label, IF condition, ELSE fallback, and ENDIF label as one visible section while still exposing the relevant lengths and labels directly. The separate right-facing block shares the same starting anchor as the left-facing block.') }}</flux:callout.text>
        @php
            $docExampleSource = file_get_contents(\Illuminate\Support\Facades\View::getFinder()->find(
                'translation-workbench::pages.tw-graph.samples.documentation.idea-to-paper.flow.flow-if-else-endif',
            ));
            if (preg_match('/^[ \t]*\{\{-- flow-if-else-endif-example-1:start --\}\}\R(.*?)^[ \t]*\{\{-- flow-if-else-endif-example-1:end --\}\}/ms', $docExampleSource, $docExample1Match) !== 1) {
                throw new \LogicException('Missing flow-if-else-endif-example-1 source markers.');
            }
        ...
        <flux:heading class="mt-4" size="sm">{{ __('Flow if else endif props') }}</flux:heading>
    ...
    <flux:callout color="zinc" icon="eye" class="min-w-0">
        ...
        <x-translation-workbench::ui.tw-graph.preview-tools :dev="$dev ?? true" :coordinates="$coordinates ?? false">
                    <div class="mt-3 overflow-x-auto rounded-lg border border-zinc-200 bg-white/70 dark:border-zinc-700 dark:bg-zinc-900/40">
                        {{-- flow-if-else-endif-example-1:start --}}
                ...
                {{-- flow-if-else-endif-example-1:end --}}
                    </div>
                </x-translation-workbench::ui.tw-graph.preview-tools>
        <flux:field class="m-3 flex justify-end font-mono text-xs text-zinc-400">.../flow/flow-if-else-endif.blade.php</flux:field>
    </flux:callout>
</section>
