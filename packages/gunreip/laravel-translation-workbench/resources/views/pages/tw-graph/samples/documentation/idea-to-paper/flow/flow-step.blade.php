<section class="mt-4 grid min-w-0 gap-4 lg:grid-cols-2">
    <flux:callout color="indigo" icon="file-text" class="min-w-0">
        ...
        <flux:callout.text>{{ __('Flow step adds a labeled process step after an existing flow anchor. The centered step label describes the shared process state; optional node labels at the end anchor carry concrete facts about the next hand-authored decision point.') }}</flux:callout.text>
        @php
            ...
            if (preg_match('/^[ \t]*\{\{-- flow-step-example-1:start --\}\}\R(.*?)^[ \t]*\{\{-- flow-step-example-1:end --\}\}/ms', $docExampleSource, $docExample1Match) !== 1) {
                throw new \LogicException('Missing flow-step-example-1 source markers.');
            }
        ...
        <flux:heading class="mt-4" size="sm">{{ __('Flow step props') }}</flux:heading>
    ...
    <flux:callout color="zinc" icon="eye" class="min-w-0">
        ...
        <flux:field class="m-3 flex justify-end font-mono text-xs text-zinc-400">.../flow/flow-step.blade.php</flux:field>
    </flux:callout>
</section>
