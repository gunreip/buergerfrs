{{-- resources/views/components/admin/partials/activity-log/table/⚡all.blade.php --}}
            ...
            <flux:table.rows>
                @forelse ($activityLogs as $activityLog)
                    @php
                        ...
                        $actorCwd = trim((string) ($actor['cwd'] ?? ''));

                        $isAppLog =
                            $subjectType !== '' || $subjectId !== null || $causerType !== '' || $causerId !== null;
                        $isArtisanLog = !$isAppLog && $actorPhpSapi === 'cli';

                        $activityLogType = $isAppLog ? 'app' : ($isArtisanLog ? 'artisan' : 'system');

                        $activityLogTypeLabel = match ($activityLogType) {
                            'app' => __('App'),
                            'artisan' => __('Artisan'),
                            default => __('ui.labels.label'),
                        };
                    ...
                    {{-- Table-Row --}}
                    <flux:table.row wire:key="activity-log-all-{{ $id }}">
                        ...
                        {{-- Table-Cell Context --}}
                        <flux:table.cell class="align-top">
                            @if ($activityLogType === 'app')
                                <div class="flex max-w-xl flex-col gap-2">
                                    <div class="flex flex-wrap items-center gap-1">
                                        <span class="mr-1 text-xs font-semibold uppercase tracking-wide text-zinc-400">
                                            {{ __('Subject') }}
                                        </span>
                            ...
                            @elseif ($activityLogType === 'artisan')
                                <div class="flex max-w-xl flex-col gap-1">
                                    ...
                                    @if ($actorCwd !== '')
                                        <code class="wrap-anywhere block text-xs text-zinc-500 dark:text-zinc-400">
                                            {{ $actorCwd }}
                                        </code>
                                    @endif
                                </div>
                    ...
                    <flux:table.row>
                        ...
                        <flux:table.cell colspan="10">
                            <div class="py-8 text-center text-sm text-zinc-500 dark:text-zinc-400">
                                {{ __('No activity log entries found for the selected filters.') }}
                            </div>
                        </flux:table.cell>
                    </flux:table.row>
                @endforelse
