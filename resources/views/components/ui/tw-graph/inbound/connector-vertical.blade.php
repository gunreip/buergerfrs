{{-- resources/views/components/ui/tw-graph/inbound/connector-vertical.blade.php --}}
{{--
    Usage:
    <x-ui.tw-graph.inbound.connector-vertical side="left" length="6rem" branch-offset="4.5rem" />
    <x-ui.tw-graph.inbound.connector-vertical length="4rem" branch-offset="3rem" color="cyan" node />
    <x-ui.tw-graph.inbound.connector-vertical length="4rem" branch-offset="3rem" text="Root key" badge-color="cyan" />

    Props:
    side="left|right" Default: right
    length="2rem|4rem|6rem|..."
    branch-offset="2rem|3rem|4.5rem|..."
    connector-length="2rem|3rem|4.5rem|..." Deprecated alias for branch-offset.
...
@php
    ...
    $showNode = filter_var($node, FILTER_VALIDATE_BOOLEAN) || $hasText || $hasOrigin;
...
<div
    {{ $attributes->class([
        ...
        'tw-graph-inbound-connector-vertical-group',
        'tw-graph-inbound-connector-vertical-group-left' => $side === 'left',
        'tw-graph-inbound-connector-vertical-group-right' => $side === 'right',
    ])->style([
    ...
    @endif
>
    <div class="tw-graph-inbound-connector-vertical"></div>

    @if ($showNode)
        <div class="tw-graph-branch-node tw-graph-inbound-connector-vertical-node">
            @if ($hasText)
